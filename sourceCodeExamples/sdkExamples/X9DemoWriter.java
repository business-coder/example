...
/**
 * X9DemoWriter creates a output x9.37 file from individual items using x9writer. Input items would
 * typically be extracted from an input source such as a database or xml file. X9Writer uses an
 * externally defined headerXml file to assign all attributes of the constructed x9.37 file.
 *
 ...
 */
public final class X9DemoWriter {
	 ...
	 * Constants.
	 */
	private static final String X9DEMOWRITER = "X9DemoWriter";
	 ...
	 * Logger instance.
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(X9DemoWriter.class);

	/*
	 * X9DemoWriter Constructor.
	 */
	public X9DemoWriter() {
		/*
		 ...
		 */
		X9SdkRoot.logStartupEnvironment(X9DEMOWRITER);
	...
	private void process() {
		try {
			 ...
			 */
			final File inputFile = new File(baseFolder, "Test file with 100 checks translate.csv");
	...
	private void createX937File(final List<X9Item937> itemList,
		...
		try (final X9Writer x9writer = new X9Writer(sdkBase)) {
			 ...
			 */
			final File outputFile = new File(baseFolder, "x9demoWriter.x9");
	...
	public static void main(final String[] args) {
		...
		LOGGER.info(X9DEMOWRITER + " started");
		try {
			final X9DemoWriter x9demoWriter = new X9DemoWriter();
