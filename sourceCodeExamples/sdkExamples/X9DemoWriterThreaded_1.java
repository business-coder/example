...
import com.x9ware.tools.X9Thread;

/**
 * X9DemoWriterThreaded is an example of x9writer being run from multiple background threads
 * concurrently. The purpose of this example is to demonstrate the ability of the SDK to run in a
 * high volume environment where a large number of files are created concurrently. This example is
 * still simplistic, in the sense that the same csv data is used to repetitively generate the
 * created x9.37 files. In that sense, the runtime is unrealistically low, given that we are reading
 * the same images (and hence most probably cached by underlying hardware). But despite those
 * limitations, this remains a good test of x9writer parallel operations.
 *
 ...
 */
public final class X9DemoWriterThreaded {
	 ...
	 * Constants.
	 */
	private static final String X9DEMOWRITER_THREADED = "X9DemoWriterThreaded";
	 ...
	 * Logger instance.
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(X9DemoWriterThreaded.class);

	/*
	 * X9DemoWriterThreaded Constructor.
	 */
	public X9DemoWriterThreaded() {
		/*
	...
	/**
	 * Exercise the SDK multi-threading capabilities by starting multiple threads. We will create
	 * the same x9.37 file repetitively in the interest of simplicity for this example.
	 */
	private void process() {
		/*
		 * Set logging mode as quiet.
		 */
		 ...
		 */
		X9SdkRoot.logStartupEnvironment(X9DEMOWRITER_THREADED);
		X9SdkRoot.loadXmlConfigurationFiles();

		/*
		 * Start concurrent threads and wait for them to complete.
		 */
		...
		try {
			 ...
			 */
			final File headersFile = new File(baseFolder, "x9headers.xml");
			...
			/*
			 * Create a count down latch that will wait for all background tasks to complete.
			 */
			final X9CountDownLatch waitLatch = new X9CountDownLatch(THREAD_COUNT);
			...
			for (int i = 0; i < THREAD_COUNT; i++) {
				...
				final Runnable task = () -> {
					createX937File(itemList, x9headerXml937, threadNumber);
				};
				X9Thread.startAndPost(task, waitLatch);
			}
			...
			/*
			 * Log the total time to create all x9.37 output files.
			 */
			LOGGER.info("function({}) completed elapsed({})", X9DEMOWRITER_THREADED,
					X9Task.formatElapsedSeconds(startTime));
		...
		try (final X9CsvReader csvReader = new X9CsvReader(inputFile)) {
			...
			while (csvLine != null && csvLine.isPopulated() && !isEndEncountered) {
				...
				if (isEndEncountered) {
					 ...
					 */
					LOGGER.info("adding csvLine({}) record({})", csvLine.getLineNumber(),
							StringUtils.join(record, COMMA));
				} else {
	 ...
	 *            thread number
	 */
	private void createX937File(final List<X9Item937> itemList, final X9HeaderXml937 x9headerXml937,
			final int threadNumber) {
		/*
		 * Create sdk instance and set options.
		 */
		final X9SdkBase sdkBase = new X9SdkBase();
		LOGGER.info("starting threadNumber({}) sdkBase({})", threadNumber, sdkBase);
		...
		try (final X9SdkIO sdkIO = sdk.getSdkIO();
				final X9Writer x9writer = new X9Writer(sdkBase)) {
			/*
			 * Open the output file.
			 */
			final File outputFile = new File(baseFolder,
					"x9demoWriterThreaded_file_" + (threadNumber + 1) + ".x9");
	...
	public static void main(final String[] args) {
		...
		LOGGER.info(X9DEMOWRITER_THREADED + " started");
		try {
			final X9DemoWriterThreaded x9demoWriter = new X9DemoWriterThreaded();
			x9demoWriter.process();
